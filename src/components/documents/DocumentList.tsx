...
export function DocumentList({
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return result;
  }, [documents, searchQuery, selectedTypes, selectedUser, selectedKeywords, sortBy]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['pdf', 'image', 'document', 'spreadsheet', 'text', 'code'].map((type) => (
                      <Button
                        key={type}
                        variant={selectedTypes.includes(type as FileCategory) ? 'default' : 'outline'}
                        size="sm"
                        onClick={() => toggleType(type as FileCategory)}
                        className="text-xs"
                      >
                        {getCategoryLabel(type as FileCategory).replace(' Files', '').replace('Office ', '')}
                      </Button>
                    ))}
                  </div>
                </div>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-lg font-medium mb-2">
              {searchQuery || activeFilterCount > 0 ? 'No matching documents' : 'No documents yet'}
            </h3>
...
function getDocumentCategory(fileType: string): FileCategory {
  const typeMap: Record<string, FileCategory> = {
    ...
    txt: 'text', md: 'text',
    json: 'code', js: 'code', ts: 'code', html: 'code', css: 'code', xml: 'code',
