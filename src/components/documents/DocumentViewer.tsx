...
import { getFileTypeInfo, canPreviewFile, formatFileSize } from '../../utils/fileTypes';
...
export function DocumentViewer({ 
  ...
  useEffect(() => {
    if (fileInfo.category === 'text' || fileInfo.category === 'code') {
      ...
      setTimeout(() => {
        const mockContent = getMockTextContent(document.fileType, document.title);
  ...
  const handleImageError = () => {
    setIsLoading(false);
    setLoadError('Failed to load image. The file may be corrupted or unavailable.');
  };
  ...
  const renderPreview = () => {
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full gap-4 p-8">
          ...
          <p className="text-muted-foreground text-center max-w-md">
            {fileInfo.label} files cannot be previewed in the browser. 
            You can download the file to view it in a compatible application.
          </p>
    ...
    if (accessStatus.type === 'expired' || accessStatus.type === 'not-started') {
      return (
        <div className="flex flex-col items-center justify-center h-full gap-4 p-8">
          <div className={cn(
            "p-4 rounded-full",
            accessStatus.type === 'expired' ? 'bg-destructive/10' : 'bg-yellow-500/10'
          )}>
            <Clock className={cn(
              "w-16 h-16",
              accessStatus.type === 'expired' ? 'text-destructive' : 'text-yellow-500'
            )} />
          </div>
          <h3 className="text-xl font-semibold">
            {accessStatus.type === 'expired' ? 'Access Expired' : 'Access Not Yet Available'}
          </h3>
          <p className="text-muted-foreground text-center max-w-md">
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <div className="space-y-3">
          ...
          <InfoRow icon={HardDrive} label="Size" value={formatFileSize(document.fileSize)} />
          <InfoRow 
            icon={Calendar} 
            label="Uploaded" 
            value={format(new Date(document.uploadDate), 'MMM d, yyyy \'at\' h:mm a')}
          />
          <InfoRow 
            icon={fileInfo.icon} 
            label="Type" 
  ...
  return (
    ...
    >
      ...
      <div className="w-80 bg-card border-l flex flex-col">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ViewerTab)} className="flex-1 flex flex-col">
...
function getAccessStatus(document: AuthDocument) {
  ...
  const startDate = document.accessStart ? new Date(document.accessStart) : null;
