  ...
  const processFile = async (uploadFile: UploadFile) => {
    ...
    if (file.size > maxFileSize) {
      setFiles(prev => prev.map(f => 
        f.id === id 
          ? { ...f, status: 'error', error: `File too large. Maximum size is ${formatFileSize(maxFileSize)}` }
          : f
      ));
      return;
    }
      ...
      setFiles(prev => prev.map(f => 
        f.id === id 
          ? { 
              ...f, 
              status: 'complete',
              aiSummary: 'Document analyzed successfully. AI has extracted key information and generated searchable metadata.',
              aiKeywords: ['document', 'uploaded', file.name.split('.')[0].toLowerCase().replace(/_/g, ' ')],
            }
          : f
      ));
    ...
    <div className="space-y-4">
      ...
      <Card
        ...
        className={cn(
          ...
          !isDragActive && !isDragReject && "hover:border-primary/50 hover:bg-muted/50",
