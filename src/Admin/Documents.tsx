...
import { DocumentList, UploadZone, DocumentViewer, ConfirmDialog, AnalyticsDashboard } from '../components/documents';
import { useTheme } from '../Zustand/themeStore';
import { useAuth } from '../context/AuthContext';
import { mockApiService } from '../services/mockApi';
import type { AuthDocument, DocumentFilter } from '../types/auth';
...
export const Documents = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Tabs */}
      <div className={`flex space-x-1 p-1 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}>
        {tabs.map(tab => (
          <button
            key={tab.id}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white'} shadow`}>
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Total Documents</p>
              <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {documents.length}
              </p>
            </div>
            <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white'} shadow`}>
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Total Size</p>
              <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {(documents.reduce((acc, d) => acc + d.fileSize, 0) / (1024 * 1024)).toFixed(1)} MB
              </p>
            </div>
            <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white'} shadow`}>
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Unique Owners</p>
              <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {new Set(documents.map(d => d.ownerId)).size}
              </p>
            </div>
            <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white'} shadow`}>
              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>File Types</p>
              <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              As an admin, you can upload documents that will be attributed to your account.
            </p>
          </div>
          <UploadZone
